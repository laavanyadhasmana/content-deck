...
const toggleVisibility = async (type, id, item) => {
  playSound('click');
  console.log('Toggling visibility:', { type, id, item });
  ...
  try {
    ...
    const result = await api.call(`${endpoint}/${id}`, { 
      method: 'PUT', 
                    ...
                    <div className="flex justify-between">
                      ...
                      <div className="flex flex-col gap-2 ml-6">
                        <button 
                          onClick={(e) => {
                            e.stopPropagation();
                            toggleVisibility('blog', blog.id, blog);
                          }} 
                          type="button"
  ...
  <div className="animate-fadeIn">
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {filteredMovies.map((movie, idx) => (
        <div key={movie.id} 
          className={`relative rounded-2xl ${darkMode ? 'bg-slate-800/50 border-purple-500/20 hover:border-pink-500/50' : 'bg-white/80 border-purple-200 hover:border-pink-400'} backdrop-blur-xl border p-6 transition-all duration-300 hover:shadow-2xl`}
          style={{ animationDelay: `${idx * 0.1}s` }}>
          ...
          <div className="flex items-center gap-1 mb-3">
            {[...Array(5)].map((_, i) => (
              <Star key={i} className={`w-5 h-5 ${i < movie.rating ? 'text-yellow-400 fill-yellow-400' : 'text-slate-600'} transition-all`} 
                style={{ transitionDelay: `${i * 50}ms` }} />
            ))}
          </div>

          <p className={`${darkMode ? 'text-slate-300' : 'text-slate-700'} text-sm leading-relaxed mb-4`}>{movie.notes}</p>
          ...
          <div className="flex gap-2 relative z-10">
            <button 
              onClick={(e) => {
                e.preventDefault();
                e.stopPropagation();
                toggleVisibility('movie', movie.id, movie);
              }} 
              type="button"
              className={`flex-1 px-3 py-2 ${darkMode ? 'bg-slate-900/80 hover:bg-slate-700' : 'bg-white hover:bg-slate-50'} rounded-lg transition-all border ${darkMode ? 'border-purple-500/30' : 'border-purple-200'} flex items-center justify-center gap-1`}>
              {movie.isPublic ? <Eye className="w-4 h-4 text-green-400" /> : <EyeOff className="w-4 h-4 text-slate-500" />}
              <span className="text-xs">{movie.isPublic ? 'Public' : 'Private'}</span>
            </button>
